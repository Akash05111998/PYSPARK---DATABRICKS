...
CREATE TABLE Customer (
    ...
    Customer_name VARCHAR(100) CHECK (Customer_name NOT LIKE '%[^a-zA-Z ]%'),
    Aadhar_card VARCHAR(12) UNIQUE NOT NULL,
    Mobile_number VARCHAR(10) UNIQUE NOT NULL,
    Date_of_birth DATE CHECK (DATEDIFF(YEAR, Date_of_birth, GETDATE()) > 15),
    Address VARCHAR(255),
    Address_type_code CHAR(1) CHECK (Address_type_code IN ('B', 'H', 'O')),
    State_code CHAR(2) CHECK (State_code IN ('MH', 'KA'))
);

CREATE TABLE AddressType (
    Address_type_code CHAR(1) PRIMARY KEY CHECK (Address_type_code IN ('B', 'H', 'O')),
    Address_type_description VARCHAR(50)
);

INSERT INTO AddressType (Address_type_code, Address_type_description) VALUES
('B', 'Business'),
('H', 'Home'),
('O', 'Office');
...
INSERT INTO StateInfo (State_id, State_name, Country_code) VALUES
(1, 'Maharashtra', 'IN'),
(2, 'Karnataka', 'IN');


ALTER TABLE Customer
ADD CONSTRAINT FK_Customer_AddressType FOREIGN KEY (Address_type_code) REFERENCES AddressType(Address_type_code);

ALTER TABLE Customer
ADD CONSTRAINT FK_Customer_StateInfo FOREIGN KEY (State_code) REFERENCES StateInfo(State_id);


EXEC sp_rename 'Customer.Customer_name', 'c_name', 'COLUMN';


INSERT INTO Customer (Customer_name, Aadhar_card, Mobile_number, Date_of_birth, Address, Address_type_code, State_code) VALUES
('John Doe', '123456789012', '9876543210', '2000-01-01', '123 Main St', 'H', 'MH'),
('Jane Smith', '234567890123', '8765432109', '1995-05-15', '456 Elm St', 'O', 'KA');
